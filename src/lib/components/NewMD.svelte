...
<script lang="ts">
	 ...
	 */
	const insertText = async (el: ContentElement, selectionStart: number, selectionEnd: number) => {
		if (el.display === 'inline') {
			// insert at current position
			textAreaValue = `${textAreaValue.slice(0, selectionEnd)}${el.text}${textAreaValue.slice(
				selectionEnd
			)}`;
		} else if (el.display === 'wrap') {
			textAreaValue = insertChar(textAreaValue, el.text, selectionStart);
			textAreaValue = insertChar(textAreaValue, keyPairs[el.text], selectionEnd + el.text.length);
...
<div id={controlsId} class={controlsClass}>
	{#each contentElements as el}
		<button class={buttonClass} on:click={() => addContent(el)} aria-label={el.name}>
